...
const uiComponentScaffold = async () => {
    ...
    if (sh.ls(dir).find(file => file === `${name}.tsx`)) {
        const { overwrite } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'overwrite',
                message: `components/ui/${name}/${name}.tsx exists. Overwrite?`,
                default: false,
            },
        ]);
        if (!overwrite) {
    ...
    js.to(path.resolve(dir, `${name}.tsx`));
    css.to(path.resolve(dir, `${name}.module.css`));
    index.to(path.resolve(dir, `index.ts`));

    const check = chalk.green.bold('✓');
    console.log('\nFiles written:');
    console.log(`- components/ui/${name}/index.ts ${check}`);
    console.log(`- components/ui/${name}/${name}.tsx ${check}`);
...
const pageComponentScaffold = async () => {
    ...
    const routeFile = route.match(/\.tsx$/) ? route : `${route}.tsx`;
    ...
    if (sh.ls(compDir).find(file => file === `${name}.tsx`)) {
        const { overwrite } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'overwrite',
                message: `components/pages/${name}/${name}.tsx exists. Overwrite?`,
                default: false,
            },
        ]);
        if (!overwrite) {
    ...
    js.to(path.resolve(compDir, `${name}.tsx`));
    css.to(path.resolve(compDir, `${name}.module.css`));
    index.to(path.resolve(compDir, `index.ts`));

    const check = chalk.green.bold('✓');
    console.log('\nFiles written:');
    console.log(`- pages/${routeFile} ${check}`);
    console.log(`- components/pages/${name}/index.ts ${check}`);
    console.log(`- components/pages/${name}/${name}.tsx ${check}`);
